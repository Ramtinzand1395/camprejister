...
const Rezayatname = () => {
  ...
  const [haveIllness, setHaveIllness] = useState(null);
  const [bimari, setBimari] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    doc.text(
      `حساسیت: ${
        haveHassasiat ? hassasiat || "ذکر نشده" : "هیچ حساسیتی ندارم"
      }`,
      pageWidth - 40,
      y,
      { align: "right" }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-xl mt-10">
      ...
      <ul className="list-disc list-inside mt-2 space-y-1">
        ...
        <li>
          رعایت نظم، مقررات اردو و دستورات مسئولین برگزاری اردو بر عهده فرزندم
          است.
        </li>
        <li>
          هرگونه مسئولیت ناشی از عدم رعایت مقررات توسط فرزندم، بر عهده اینجانب
      ...
      <div className="mt-6">
        <label className="block mb-2 font-medium">امضا ولی:</label>
        <p className="text-xs text-red-600 my-2">با موس یا انگشت داخل باکس بکشید</p>
        <Canvas canvasRef={canvasRef} />
      </div>
